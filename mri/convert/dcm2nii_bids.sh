...
for subj in `ls -d /mindhive/xnat/dicom_storage/BILD/Kids/bch0*`
do
subj=$(basename $subj)
mkdir ${temp_dir}/${subj}_dicom
cd ${temp_dir}/${subj}_dicom
echo '#!/bin/bash' > dicom_run.sh
echo "singularity exec -B /mindhive/xnat/dicom_storage/BILD/Kids:/dicomdir -B /om/user/zqi/projects/bildmeg/subject/:/output -B /om/user/zqi/projects/bildmeg/script/bildmeg/mri/convert/:/mnt -c /storage/gablab001/data/singularity-images/heudiconv/nipy/heudiconv heudiconv -d /dicomdir/%s/dicom/*.dcm -c dcm2niix -o /output -f /mnt/heuristic_bildmeg_bids.py -s ${subj} -b" >> dicom_run.sh
done
for subj in `ls -d /mindhive/xnat/dicom_storage/BILD/Kids/bch0*`
do
subj=$(basename $subj)
